...
export default function CuisineSection() {
  ...
  return (
    <section
      ref={ref}
      className={`${mono.variable} ${body.variable} relative w-full overflow-hidden bg-[#722F38] py-16 text-[#F5EAD6] md:py-20`}
    >
        ...
        <div className="anzar-marquee flex w-max gap-5 group-hover:[animation-play-state:paused]">
          {loop.map((s, i) => (
            ...
            >
              ...
              />
              <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(0deg,rgba(40,10,16,0.35)_0%,transparent_50%)]" />
            </div>
          ))}
        </div>

        {/* edge fades into the wine background */}
        <div className="pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-[#722F38] to-transparent md:w-28" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-[#722F38] to-transparent md:w-28" />
      </div>
